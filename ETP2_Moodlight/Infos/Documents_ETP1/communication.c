...
uint16_t TX_buf = 0;						///< transmit buffer
//char RX_buf[COM_BUF_SIZE] = "";				///< receive buffer
uint16_t RX_buf = 0;						///< receive buffer
uint8_t TX_index = COM_BUF_SIZE;						///< transmit buffer index
uint8_t RX_index = COM_BUF_SIZE;						///< receive buffer index
...
//	RX_buf[0] = '\0';
	RX_buf = 0;
	RX_index = COM_BUF_SIZE;
//	COM_RX_Data[0] = '\0';
	COM_RX_Data = 0;
	COM_RX_Available_Flag = false;
//	TX_buf[0] = '\0';
	TX_buf = 0;
	TX_index = COM_BUF_SIZE;
//	COM_TX_Data[0] = '\0';
	COM_TX_Data = 0;
...
void COM_TX_PutData(uint16_t * data) {
	TX_buf = *data;				// Copy data to TX buffer
	TX_index = COM_BUF_SIZE;							// Start to transmit with first byte
...
//		char new_char = COM_LEUART->RXDATA;		// Fetch the newly received char
		uint16_t new_byte = (uint16_t)(COM_LEUART->RXDATA);		// Fetch the newly received char
//		if ((COM_END_OF_STRING != new_byte) && (COM_BUF_SIZE > RX_index)) {	// A "normal" char
		if ((0 < RX_index)) {	//
			RX_buf |= (new_byte << ((RX_index-1) * 8));		// store the received byte
			RX_index--;						// and increment the index
		} else {							// End of string is reached
			COM_RX_Data = RX_buf;			// copy new request
			COM_RX_Available_Flag = true;	// ready for processing
			RX_index = COM_BUF_SIZE;					// Start a new string
		}
	}

	if (COM_LEUART->STATUS & LEUART_STATUS_TXBL) {	// Check TX buffer level status
		if (COM_TX_Busy_Flag) {					// For synchronization
			uint16_t next_byte = (TX_buf >> ((TX_index-1) * 8)) ;
			if (0 < TX_index) {
				COM_LEUART->TXDATA = (uint8_t) next_byte;	// Send the byte
...
//				COM_LEUART->TXDATA = (uint8_t) COM_END_OF_STRING;	// Send end of string char
				...
				TX_index = COM_BUF_SIZE;					// and start anew
